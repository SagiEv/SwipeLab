...
* Backend: Resurrected and upgraded the credibility system — `CredibilityService` was dead code (never called). Fixed 7 bugs: added `CredibilityEventListener` to wire into `ClassificationSubmittedEvent`, implemented composite 0-100 score (40% gold accuracy + 35% majority agreement + 25% expert kappa), fixed image-query granularity per `(imageId, querySpecies)`, added malicious labeling detection, clamped score to [0,100], reset default to 50.0, added Flyway V14 migration, 36 new/updated tests all passing (PR #267).
